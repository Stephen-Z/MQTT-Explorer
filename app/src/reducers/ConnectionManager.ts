...
function updateConnection(state: ConnectionManagerState, action: UpdateConnection): ConnectionManagerState {
	...
	if (changeSet.clientId !== undefined) {
		const clientid = changeSet.clientId;
		if (connection.subscriptions.length === 0) {
			const subs: Subscription = { topic: `/${clientid}/#`, qos: 0 }
			connection.subscriptions.push(subs)
		}
		changeSet.clientId += new Date().getTime()
	}
